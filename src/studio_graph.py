...
async def ado_push_node(state: PipelineGraphState) -> dict:
    ...
    try:
        ...
        return {
            "current_stage": "test_case_creation",
            "ado_results": result,
...
# ============================================================================
# NODE FUNCTION - TEST CASE CREATION
# ============================================================================

async def test_case_creation_node(state: PipelineGraphState) -> dict:
    """Create test cases in Azure DevOps for each user story after work items are pushed."""
    ado_client = get_ado_client()
    if not ado_client:
        return {
            "current_stage": "architecture",
            "messages": [{"role": "system", "content": "⚠️ ADO client not configured - skipping test case creation", "stage": "test_case_creation"}]
        }
    stories = state.get("user_stories", [])
    created_cases = []
    for story in stories:
        title = story.get("title", "Untitled Test Case")
        ...
        try:
            result = await ado_client.call_tool("wit_create_work_item", {
                ...
                "fields": [
                    ...
                    {"name": "Microsoft.VSTS.TCM.Steps", "value": steps},
                ],
            })
            created_cases.append({"story_id": story.get("id"), "ado_id": result.get("id"), "result": result})
        except Exception as e:
            created_cases.append({"story_id": story.get("id"), "error": str(e)})
    return {
        "current_stage": "architecture",
        "created_test_cases": created_cases,
        "messages": [{"role": "assistant", "content": f"✅ Created {len(created_cases)} test cases in ADO", "stage": "test_case_creation"}]
    }
...
def build_graph():
    ...
    # Test Case Creation stage
    builder.add_node("test_case_creation", test_case_creation_node)
    ...
    builder.add_edge("ado_push", "test_case_creation")
    builder.add_edge("test_case_creation", "test_plan_confirm")
